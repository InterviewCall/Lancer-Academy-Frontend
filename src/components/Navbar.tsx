...
import { navbarItems } from "@/utits/content";
    ...
    <div className="fixed z-[10] sm:justify-between w-full flex sm:grid sm:gap-3 sm:grid-cols-[9fr_1fr] sm:pl-3 sm:pr-1 pt-2">
      <div
        className={
          ...
          (isScrolled
            ? `bg-white shadow-lg ${isNavbarOpen ? " " : " hidden "} sm:flex`
            : "bg-transparent ") +
      ...
      >
        ...
        <div className="hidden sm:flex items-center gap-4 font-light text-sm pl-20 ">
          {navbarItems.map((item, index) => (
            <a
              key={index}
              href={item.href}
              className="cursor-pointer hover:bg-[#dcdcdc] p-2 hover:rounded-lg"
            >
              {item.item}
            </a>
          ))}
        ...
        <div className=" items-center opacity-0">
          ...
          {isNavbarOpen ? (
            ...
            <FiMenu
              className="block sm:hidden h-4 w-4 hover:cursor-pointer bg-white"
              onClick={() => setIsNavbarOpen((prev) => !prev)}
            />
          )}
        </div>
            ...
            <div className="flex flex-col w-full h-full">
              ...
              <div className="flex-1 h-full overflow-y-hidden mx-5">
                <div className="overflow-y-scroll h-full bg-transparent text-black px-6 py-4 space-y-4">
                  ...
                  <div className="space-y-2 text-base font-medium pl-5 flex flex-col">
                    {navbarItems.map((item, index) => (
                      <a key={index} href={item.href} onClick={() => setIsNavbarOpen(false)} className="cursor-pointer">
                        {item.item}
                      </a>
                    ))}
                    {/* <li>Intro</li>
                  ...
                  <ul className="space-y-2 text-base font-medium pl-5">
                    <li>
                      Intermediate Software Engineering Program (7 months)
                    </li>
      ...
      <div className="w-full flex items-stretch justify-end">
        ...
        <div
          className={
            "flex sm:w-full rounded-lg p-4 sm:p-0 items-center " +
            (isScrolled ? " max-md:bg-white" : " bg-transparent ")
          }
        >
          <button
            onClick={() => {
