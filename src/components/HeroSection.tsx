...
//         <div className="order-2 md:order-1 md:basis-[45%] lg:basis-[44%] flex items-center justify-center mt-6 md:mt-0">
//     <div className="relative w-full max-w-[560px] lg:max-w-[600px] aspect-[3/4]">
//       {isVideoActive ? (
//         <video
//           className="absolute inset-0 w-full h-full rounded-xl object-cover"
...
//       <button
//         className="absolute bottom-[10%] left-1/2 -translate-x-1/2 bg-[#65bd82] p-7 md:p-9 rounded-full flex items-center justify-center cursor-pointer"
//         onClick={() => setIsVideoActive(prev => !prev)}
//       >
//         {isVideoActive ? <FaPause size={26} /> : <FaPlay size={26} />}
//       </button>
//     </div>
//   </div>

//       <div className="md:basis-3/5 md:order-2 w-full">
    ...
    <section className="flex flex-col md:flex-row items-start justify-between gap-10 mt-10 pt-14 mb-10 max-md:px-6">
      ...
      <div className="order-1 md:order-2 md:basis-[55%] w-full">
        ...
        <div className="w-full flex justify-center md:justify-start">
          <div className="flex items-center bg-[#B8E9C8] my-1 rounded-lg px-3 py-2 md:px-4 gap-4">
            {/* Intermediate */}
            <div className="flex items-center gap-1 text-sm md:text-base font-semibold whitespace-nowrap">
              <FaCircleHalfStroke size={14} className="text-black" />
              <span>Intermediate:</span>
              <span className="text-[#318f51]">7 months</span>
            </div>
        ...
        {/* HEADING */}
        <div className="relative">
          <h1 className="text-center md:text-start text-[clamp(3.5rem,7vw,6rem)] font-[700] pt-5 leading-[0.9] z-[10]">
            Full Stack and AI Software Engineering Program
          </h1>
        </div>
        ...
        <p className="font-[400] text-center md:text-start leading-snug tracking-[-0.015em] pt-5 lg:pt-8 px-4 md:px-0 text-base sm:text-lg md:text-xl lg:text-2xl">
          ...
          <span className="font-semibold">4-month advanced</span> or{" "}
          <span className="font-semibold">7-month intermediate</span> track –– based on
          your schedule.
        </p>

        {/* CTA + COHORT TAG */}
        <div className="mt-11 mb-5 flex flex-col md:flex-row items-center gap-x-2 gap-y-2 md:items-stretch">
          <BookACallButton />
          <div className="flex justify-center">
            <Image
              ...
              width={100}
              height={100}
              className="h-auto md:h-[100%] w-9/10 md:w-[100%]"
            />
          </div>
        </div>
      </div>

      {/* MEDIA BLOCK – SINGLE VERSION FOR MOBILE + DESKTOP */}
      <div className="order-2 md:order-1 md:basis-[45%] lg:basis-[44%] w-full flex items-center justify-center mt-6 md:mt-0">
        <div className="relative w-full max-w-[420px] sm:max-w-[560px] lg:max-w-[600px] aspect-[3/4]">
          {isVideoActive ? (
            <video
              ref={videoRef}
              src={heroSectionVideoUrl}
          ...
          <button
            className="absolute bottom-[10%] left-1/2 -translate-x-1/2 bg-[#65bd82] p-6 md:p-9 rounded-full flex items-center justify-center cursor-pointer"
            onClick={() => setIsVideoActive(prev => !prev)}
          >
            {isVideoActive ? <FaPause size={26} /> : <FaPlay size={26} />}
          </button>
